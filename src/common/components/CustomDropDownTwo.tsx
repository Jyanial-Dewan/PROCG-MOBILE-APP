...
const CustomDropDownNew = observer(
  ({
    data,
    total,
    setValue,
    ...
    const messageGroup = [
      {
        label: 'Inbox',
        total: messageStore.totalReceived,
        icon: <Feather name="inbox" size={24} color={COLORS.darkGray} />,
      },
      {
        label: 'Sent',
        total: messageStore.totalSent,
        icon: (
          <Ionicons
      ...
      {
        label: 'Drafts',
        total: messageStore.totalDraft,
        icon: (
          <SimpleLineIcons name="notebook" size={24} color={COLORS.darkGray} />
        ),
      },
      {
        label: 'Recycle Bin',
        total: messageStore.totalBin,
        icon: <Feather name="trash" size={24} color={COLORS.darkGray} />,
      },
    ];
    ...
    useAsyncEffect(
      async isMounted => {
        ...
        const api_paramsReceivedTotal = {
          url: api.totalReceived + userInfo?.user_name,
          baseURL: ProcgURL,
          // isConsole: true,
          // isConsoleParams: true,
        };
        const api_paramsSentTotal = {
          url: api.totalSent + userInfo?.user_name,
          baseURL: ProcgURL,
          // isConsole: true,
          // isConsoleParams: true,
        };
        const api_paramsDraftTotal = {
          url: api.totalDraft + userInfo?.user_name,
          baseURL: ProcgURL,
          // isConsole: true,
          // isConsoleParams: true,
        };
        const api_paramsBinTotal = {
          url: api.totalRecycleBin + userInfo?.user_name,
          baseURL: ProcgURL,
          // isConsole: true,
          // isConsoleParams: true,
        };
        const res = await httpRequest(api_paramsReceivedTotal, setIsLoading);
